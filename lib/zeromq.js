...
function buf_to_hex(buffer) {
    ...
    for (let i = 0; i < buffer.byteLength; i++)
        rv[i] = buffer.getUint8(i).toString(16);
    return rv.join(" ");
}
...
function check_greeting(buffer)
{
    ...
    if (greeting[ 0] != 0xFF || // signature
        // bytes 1 through 9 are supposed to be all zero in ZMTP/2 but
    ...
    write: function write (message) {
        ...
        if (Array.isArray(message)) {
            for (let i = 0; i < message.length-1; i++)
                _this.socket.write(make_data_frame(message[i], true));
            _this.socket.write(make_data_frame(message[message.length-1],
                                               false));
        } else {
            _this.socket.write(make_data_frame(message, false));
        }
    },
