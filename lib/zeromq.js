...
const ZmqSocket = Class({
    ...
    initialize: function initialize (options) {
        ...
        this.identity = Encoder.encode(identity);
        if (identity.byteLength > 255)
            throw "Identity can be no more than 255 bytes encoded";

        // internals
        let _this = _(this);
        _this.state  = ZMTP_GREETING;
        _this.socket = new Socket();
        _this.socket.on('connect', ZmqSocket.onConnect.bind(this));
        _this.socket.on('data',    ZmqSocket.onData.bind(this));
        _this.socket.on('error',   ZmqSocket.onError.bind(this));
        _this.socket.on('close',   ZmqSocket.onClose.bind(this));
        _this.socket.on('end',     ZmqSocket.onClose.bind(this));
    ...
    onConnect: function onConnect () {
        _(this).socket.write(make_greeting(this.identity));
    }
    ...
    onError: function onError (err) {
        emit(this, 'error', "socket-level error: " + err);
        _this.state = ZMTP_CLOSED;
        _this.socket.destroy();
        emit(this, 'close');
    },
    ...
    onData: function onData (packet) {
        let _this = _(this);
        try {
            _this.inq = append_buf(this.inq, packet);
            ...
            while (let len = try_get_frame_len(_this.inq)) {
                let [type, val, more] = decode_frame(_this.inq, len);

                if (type == TYPE_DELIM) {
                    if (_this.inmsg.length !== 0 || !more)
    ...
    write: function write (message) {
        let _this = _(this);
        if (_this.state !== ZMTP_MESSAGES)
            throw "Not connected";

        _this.socket.write(make_delim_frame());

        if (Array.isArray(message)) {
            for (let i = 0; i < message.length-1; i++)
                _this.socket.write(make_frame(message[i], true));
            _this.socket.write(make_frame(message[message.length-1], false));
        } else {
            _this.socket.write(make_frame(message, false));
        }
    }
