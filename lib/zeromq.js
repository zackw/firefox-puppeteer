...
const ZmqSocket = Class({
    ...
    onConnect: function onConnect () {
        ...
        console.debug("zmq: sending greeting: ", buf_to_hex(greeting))
    ...
    onClose: function onClose () {
        ...
        if (_this.inq.byteLength > 0 || _this.inmsg.length > 0) {
            emit(this, 'error', Error("incomplete ZMTP message at EOF"));
        }
    ...
    onData: function onData (packet) {
        let _this = _(this);
        let len;
        console.debug("zmq: received " + packet.byteLength +
        ...
        try {
            ...
            if (_this.state === ZMTP_GREETING) {
                ...
                if (!len) {
                    console.debug("zmq: incomplete greeting: ",
                                  buf_to_hex(_this.inq));
                    return;
                }
                console.debug("zmq: complete greeting: ",
                              buf_to_hex(Uint8Array(_this.inq)
                                         .subarray(0, len)));
            ...
            while (len = try_get_frame_len(_this.inq)) {
                console.debug("zmq: received frame len", len);
                let [val, flags] = decode_frame(_this.inq, len);

                let delim = !(flags & 0x01);
                let more  = !(flags & 0x02);

                console.debug("zmq: received frame: ", val,
                ...
                if (delim) {
                    ...
                    if (!more) {
                        console.debug("zmq: complete message received");
    ...
    write: function write (message) {
        if (_(this).state !== ZMTP_MESSAGES) {
            throw Error("write called in state" + _(this).state);
        }
    ...
    write_frame: function write_frame (frame) {
        console.debug("zmq: send frame: " + buf_to_hex(frame))
