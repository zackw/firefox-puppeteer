...
const Socket = Class({
    ...
    onTransportStatus: function onTransportStatus (transport,
        ...
        console.debug("socket ("+previous+"): onTransportStatus", status);
        if (status !== STATUS_CONNECTED_TO)
            return;

        if (previous !== CONNECTING) {
            console.warn("socket: received STATUS_CONNECTED_TO notification "+
                         "in state", previous);
            return;
        }
        this.onConnected();
