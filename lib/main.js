...
function do_error (exc) {
    do_send("error", {
        name: exc.name,
        message: exc.message,
        detail: exc.toString(),
        traceback: traceback.fromException(exc)
    });
}

function do_quit () {
    if (control_log !== null) {
        control_log.write("--quit--\n");
        control_log.close();
    }
    if (watchdog_ticks)
        dump("\nDONE\n");
...
function recv_message (data) {
    ...
    dispatch(data).then(do_send, do_error);
}
...
function on_data (data) {
    ...
    {
        on_error(Error("message ill-formed, out of sequence, or " +
                       "for wrong client: " +
